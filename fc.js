    ...
    function tarrifHandler(wrapperEl,input) {
      ...
      const getDropdown = el => {
       ...
       const fetchData = async (code, el) => {
        ...
        if (data[0] === null) {
        	let msg = currentLanguage === 'de' ? 'Diese Postleitzahl existiert nicht' : "This postal code doesn't exist";
          setError(error, msg);
      ...
      document.body.addEventListener("click", (ev) => {
      	let dd1 = document.querySelector('.dd-1'); let dd2 = document.querySelector('.dd-2');
        if(window.innerWidth > 768 && dd2.style.display === 'none') return;
       	if(window.innerWidth <= 768 &&  dd1.style.display === 'none') return;
        if (!ev.target.closest(".tarrif-sc")) { removeDropdown(dd1); removeDropdown(dd2);
